...
/* HERO_OPEN (Shah's review, 2026-09-14; generate.py and home.css carry the
   account). The panel is full bleed from the first frame and the claim is a
   TIMED entrance on a one-shot `html.hero-in`, not a reveal welded to scroll.
   So in this mode --p is pinned at 1, --q is never written, and the claim is
   revealed by the first scroll -- or the first Tab, so a keyboard visitor
   never meets a hidden button. It never un-reveals: scrolling back to the top
   shows the finished screen, because the brief is the first impression, not a
   gesture to replay. Reduced motion and no-JS never reach this: home.css keeps
   their static hero, which already shows the claim. */
const openHero = expandHero && wrap.classList.contains('open');
let heroIn = false;
const revealHero = () => {
  if (heroIn || !openHero || reduced) return;
  heroIn = true;
  document.documentElement.classList.add('hero-in');
};
if (openHero) addEventListener('keydown', e => { if (e.key === 'Tab') revealHero(); });
...
function setProgress(p, immediate){
  ...
  if (openHero) {
    if (lastPs !== '1.0000') { lastPs = '1.0000'; heroVars.setProperty('--p', '1.0000'); }
    if (p > 0.001) revealHero();
  } else if (expandHero) {
  ...
  /* OPEN HERO: 250vh runway, Services starts rising at p = 1/3 (home.css), so
     the dim leads it in just before and is full by the time the curtain is
     three quarters up. The expand curve would start the dim at p .52 -- halfway
     up a curtain that begins at .33 -- so the hero would rise into view still
     bright. Coupled to the runway height; re-derive both together. */
  const dv = (openHero
    ? clamp01((p - 0.28) / 0.50) * 0.88
    : expandHero
